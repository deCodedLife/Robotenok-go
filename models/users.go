...
type User struct {
	...
	Login    string      `form:"login" boil:"login" json:"login" toml:"login" yaml:"login"`
	Password string      `form:"password" boil:"password" json:"password" toml:"password" yaml:"password"`
	Image    null.String `form:"image" boil:"image" json:"image,omitempty" toml:"image" yaml:"image,omitempty"`
	UserType string      `form:"user_type" boil:"user_type" json:"user_type" toml:"user_type" yaml:"user_type"`
...
type userR struct {
	...
	Images        ImageSlice        `form:"Images" boil:"Images" json:"Images" toml:"Images" yaml:"Images"`
...
var (
	userAllColumns            = []string{"id", "name", "login", "password", "image", "user_type", "active"}
	userColumnsWithoutDefault = []string{"name", "login", "password", "image"}
	userColumnsWithDefault    = []string{"id", "user_type", "active"}
...
func (o *User) Classes(mods ...qm.QueryMod) classQuery {
...
func (o *User) GroupCurators(mods ...qm.QueryMod) groupCuratorQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("`group_curators`.`user_id`=?", o.ID),
	)
...
func (userL) LoadClasses(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`classes`),
		qm.WhereIn(`classes.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadGroupCurators(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		...
		qm.WhereIn(`group_curators.user_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *User) AddClasses(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Class) error {
	var err error
...
// Sets related.R.User appropriately.
func (o *User) AddGroupCurators(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*GroupCurator) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.UserID = o.ID
